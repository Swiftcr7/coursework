...
#ifndef SERVER_BINARY_SEARCH_TREE_H
#define SERVER_BINARY_SEARCH_TREE_H
...
protected:
    ...
    protected:
        ...
        virtual void additional_working(node* current_node, node* dad, size_t& place, size_t& color_removing) const {
    ...
    public:
        virtual tvalue remove_inner(tkey const &key, node** root, std::stack<node*>* way_to_remove, memory* allocator, logger* logger, size_t& place, size_t& color_removing){
            ...
            if (current_node->left_node == nullptr && current_node->right_node == nullptr){
                if (flag != 2){
                    additional_working(current_node, way_to_remove->top(), place, color_removing);
                }
                if (flag == 1){
                ...
                if (replace_node->left_node == nullptr){
                    ...
                    additional_working(replace_node, way_to_remove->top(), place, color_removing);
                    tvalue removing_value = std::move(current_node->value);
                    current_node->key = replace_node->key;
                    current_node->value = replace_node->value;
        ...
        virtual void remove_after(tkey const& key, node** root, std::stack<node*>* way_to_remove, logger* logger, size_t& place, size_t& color_removing){
...
public:
    bool find(typename associative_container<tkey, tvalue>::key_value_pair* key_value_to_find)override{
        auto* way_to_find = new std::stack<node*>();
        bool result = _class_find->find_inner(key_value_to_find,&_root, _logger, way_to_find);
        _class_find->after_find(key_value_to_find,&_root, _logger, way_to_find);
        return result;
    }
    ...
    tvalue remove(tkey const &key) override{
        size_t place, color_removing;
        node** current_root = &_root;
        std::stack<node*> way_to_remove = std::stack<node*>();
        tvalue remove_value = _class_remove->remove_inner(key,current_root,  &way_to_remove, _allocator, _logger, place, color_removing);
        _class_remove->remove_after(key, current_root, &way_to_remove, _logger, place, color_removing);
        return remove_value;
    }

    void update_key(const tkey& key, const tvalue& value){
        node* current_node = _root;
        if (current_node == nullptr){
    ...
    tkey& find_for_read_range(const tkey& key_to_find){
        if ((_root) == nullptr) {
            throw std::logic_error("Empty");
        }
        tkey_comparer comparator = tkey_comparer();
        ...
        if (result != nullptr){
            ...
            throw std::logic_error("uncorrect range");
        }
    }
...
#endif //SERVER_BINARY_SEARCH_TREE_H
