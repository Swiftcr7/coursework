...
private:
    struct session_key final {
        ...
        session_key(const int id_s, const int& id_st, const std::string& sub, reporting_format report) :
                id_session(id_s),
                id_student(id_st),
                subject(flyweight_for_string::get_instance().get_flyweight(sub)),
                report_format(report)
        {}
        session_key& operator=(const session_key& session){
            this->id_session = session.id_session;
            this->id_student = session.id_student;
            this->subject = session.subject;
            this->report_format = session.report_format;
            return *this;
        }
    };
    ...
    struct session_value {
        ...
        session_value(const std::string& n_student, const std::string& s_student, const std::string& p_student, const std::string& e_data, const std::string& s_date, int est, const std::string& n_teacher, const std::string& s_teacher, const std::string& p_teacher) :
                name_student(flyweight_for_string::get_instance().get_flyweight(n_student)),
                surname_student(flyweight_for_string::get_instance().get_flyweight(s_student)),
                patronymic_student(flyweight_for_string::get_instance().get_flyweight(p_student)),
                exam_date(flyweight_for_string::get_instance().get_flyweight(e_data)),
                start_date(flyweight_for_string::get_instance().get_flyweight(s_date)),
                estimation(est),
                name_teacher(flyweight_for_string::get_instance().get_flyweight(n_teacher)),
                surname_teacher(flyweight_for_string::get_instance().get_flyweight(s_teacher)),
                patronymic_teacher(flyweight_for_string::get_instance().get_flyweight(p_teacher))
        {}

        session_value& operator=(const session_value& other){
            this->name_student = other.name_student;
            this->surname_student = other.surname_student;
            this->patronymic_student = other.patronymic_student;
            this->exam_date = other.exam_date;
            this->start_date = other.start_date;
            this->estimation = other.estimation;
            this->name_teacher = other.name_teacher;
            this->surname_teacher = other.surname_teacher;
            this->patronymic_teacher = other.patronymic_teacher;
            return *this;
        }

        bool operator==(const session_value& other){
            return (this->name_student == other.name_student && this->surname_student == other.surname_student && this->patronymic_student == other.patronymic_student && this->exam_date == other.exam_date && this->start_date == other.exam_date && this->estimation == other.estimation && this->name_teacher == other.name_teacher && this->surname_teacher == other.surname_teacher && this->patronymic_teacher == other.patronymic_teacher);
        }
        ...
        while (choice != "4"){
            ...
            if (choice == "1"){
                ...
                try{
                    ...
                    if (_logger != nullptr) _logger->log(er.what(), logger::severity::debug);
                }
            }else if (choice == "3"){
                try{
                    reset_database();
...
//----------------------------------------COMMANDS BEGINNING----------------------------------------------------
    ...
    void add_pool(const std::string& pool_name, size_t size, name_allocators name_allocator, allocator_mode mode = allocator_mode::no ){
        ...
        }else if (name_allocator == name_allocators::allocator_on_sorted_list){
            if (mode == allocator_mode::first){
                alloc = new allocator_on_sorted_list(size, allocator_on_sorted_list::detour::first, _logger);
            }else if (mode == allocator_mode::best){
                alloc = new allocator_on_sorted_list(size, allocator_on_sorted_list::detour::best, _logger);
            }else if (mode == allocator_mode::worst){
                alloc = new allocator_on_sorted_list(size, allocator_on_sorted_list::detour::worst, _logger);
            }else{
                throw std::logic_error("Incorrect mode");
            }
        }
    ...
    void add_collection(const std::string& pool_name, const std::string& scheme_name, const std::string& collection_name, name_tree tree_name){
        ...
        memory* alloc = _allocators_pool->get(pool_name);
        if (tree_name == name_tree::AVL_tree){
        ...
        }else if (tree_name == name_tree::red_black_tree){
            auto* tree_collection = reinterpret_cast<red_black_tree<session_key, session_value, session_key_comparer>*>(alloc->allocate(sizeof(red_black_tree<session_key, session_value, session_key_comparer>)));
            if (tree_collection == nullptr){
                throw std::logic_error("Not enough memory");
            }
            new (tree_collection) red_black_tree<session_key, session_value, session_key_comparer>(alloc, _logger);
            scheme->insert(collection_name, tree_collection);
        }else {
            auto* tree_collection = reinterpret_cast<splay_tree<session_key, session_value, session_key_comparer>*>(alloc->allocate(sizeof(splay_tree<session_key, session_value, session_key_comparer>)));
            if (tree_collection == nullptr){
                throw std::logic_error("Not enough memory");
            }
            new (tree_collection) splay_tree<session_key, session_value, session_key_comparer>(_logger, alloc);
            scheme->insert(collection_name, tree_collection);
        }
    ...
    void delete_collection(const std::string& pool_name, const std::string& scheme_name, const std::string& collection_name){
        ...
        auto begin = dynamic_cast<binary_search_tree<session_key, session_value, session_key_comparer>*>(collection)->infix_it_begin();
        auto end = dynamic_cast<binary_search_tree<session_key, session_value, session_key_comparer>*>(collection)->infix_it_end();
        for (; begin != end; begin++){
            auto& remove_value = std::get<2>(*begin);
            auto& remove_key = std::get<1>(*begin);
            flyweight_for_string::get_instance().remove_flyweight(remove_key.subject);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.name_student);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.surname_student);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.patronymic_student);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.exam_date);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.start_date);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.name_teacher);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.surname_teacher);
            flyweight_for_string::get_instance().remove_flyweight(remove_value.patronymic_teacher);
    ...
    void insert(const session_key& key, const session_value& value, const std::string& pool_name, const std::string& scheme_name, const std::string& collection_name){
        ...
        try{
            ...
            flyweight_for_string::get_instance().remove_flyweight(key.subject);
            flyweight_for_string::get_instance().remove_flyweight(value.name_student);
            flyweight_for_string::get_instance().remove_flyweight(value.surname_student);
            flyweight_for_string::get_instance().remove_flyweight(value.patronymic_student);
            flyweight_for_string::get_instance().remove_flyweight(value.exam_date);
            flyweight_for_string::get_instance().remove_flyweight(value.start_date);
            flyweight_for_string::get_instance().remove_flyweight(value.name_teacher);
            flyweight_for_string::get_instance().remove_flyweight(value.surname_teacher);
            flyweight_for_string::get_instance().remove_flyweight(value.patronymic_teacher);
            throw er;
        }
        try{
...
//            flyweight_for_string::get_instance().remove_flyweight(key.subject);
    ...
    std::queue<std::tuple<session_key, session_value>> read_range(const session_key* key1, const session_key* key2, const std::string& pool_name, const std::string& scheme_name, const std::string& collection_name){
        ...
        try{
            ...
            flyweight_for_string::get_instance().remove_flyweight((*key1).subject);
            flyweight_for_string::get_instance().remove_flyweight((*key2).subject);
            throw er;
        }
        bool flag = true;
...
//        if ((key1 != nullptr && !collection_bst->find_key(*key1))  || (key2 != nullptr && !collection_bst->find_key(*key2))){
...
//        }
        ...
        try{
            if (key1 != nullptr){
                while (comparator(std::get<1>(*it), (collection_bst)->find_for_read_range(*key1)) != 0 && it != dynamic_cast<binary_search_tree<session_key, session_value, session_key_comparer>*>(collection)->infix_it_end()){
                    it++;
                }
        ...
        if (flag){
            if (key2 != nullptr){
                while (it != collection_bst->infix_it_end() && comparator(std::get<1>(*it), *key2) < 0){
                    Queue.emplace(std::get<1>(*it), std::get<2>(*it));
                    it++;
                }
                if (it != collection_bst->infix_it_end() && comparator(std::get<1>(*it), *key2) == 0){
                    Queue.emplace(std::get<1>(*it), std::get<2>(*it));
                }
            }else{
     ...
     void saving_state(const std::string& way){
        if (_logger != nullptr) _logger->log("------------Saving state has started working------------", logger::severity::debug);
        std::ofstream file_users;
        file_users.open(way);
        if (!file_users.is_open()){
            throw std::logic_error("Unable to open the file");
        }
        std::ifstream file;
        file.open("/home/dima/CLionProjects/server/file_inner");
        std::string result;
        while (std::getline(file, result)){
            file_users<<result<<std::endl;
        }
        file_users.close();
        file.close();
        if (_logger != nullptr) _logger->log("------------Saving state has finished working------------", logger::severity::debug);
    ...
    static session_key parsing_session_key(const std::string& key_str){
        ...
        session_key result = {id_session, id_student, subject, format};
        return result;
    }
...
public:
    std::string parsing_enter(const std::string& command_str, std::string& key_str, std::string& value_str){
        size_t position1;
        size_t position2;
        std::string command;
        if (command_str.find("Add pool: ") == 0){
            ...
            }else if(name_allocator_str == "allocator_on_sorted_list"){
                name_allocator = name_allocators::allocator_on_sorted_list;
            }else{
            ...
            if (name_allocator == name_allocators::allocator_border_descriptor || name_allocator == name_allocators::allocator_on_sorted_list){
                position1 = command.find('{');
                if (position1 != 0){
                    throw std::logic_error("Incorrect input: not enough { or it is in the wrong place");
                }
            ...
            try{
                ...
                _file_inner<<command_str<<std::endl;
            }catch (const std::logic_error& er){
                throw er;
            }
            ...
            try{
                pool_name = parsing_string_format(command, "pool name");
                scheme_name = parsing_string_format(command, "scheme name");
                collection_name = parsing_string_format(command, "collection name");
            }catch (const std::logic_error& er){
                throw er;
            }
            position1 = command.find('[');
            ...
            std::string tree = command.substr(1, position2 - 1);
            if (tree.empty()){
                throw std::logic_error("Incorrect tree name");
            }
            if (tree == "AVL_tree"){
                tree_name = name_tree::AVL_tree;
            }else if (tree == "splay_tree"){
                tree_name = name_tree::splay_tree;
            }else if (tree == "red_black_tree"){
                tree_name = name_tree::red_black_tree;
            }else{
                throw std::logic_error("incorrect tree name");
            }
            try{
                add_collection(pool_name, scheme_name, collection_name, tree_name);
                _file_inner<<command_str<<std::endl;
            }catch (const std::logic_error& er){
                throw er;
            }
...
//            if (!key_str.empty()){
//                try{
//                    session_key key1 = std::move(parsing_session_key(key_str));
//                    flyweight_for_string::get_instance().remove_flyweight(key1.subject);
//                    key1_ptr = &key1;
//                }catch (const std::logic_error& er){
//                    throw er;
...
//            }
            ...
            if (!value_str.empty() && !key_str.empty()){
                try{

                    session_key key2 = std::move(parsing_session_key(value_str));
                    //flyweight_for_string::get_instance().remove_flyweight(key2.subject);
                    key2_ptr = &key2;
                    session_key key1 = std::move(parsing_session_key(key_str));
                    //flyweight_for_string::get_instance().remove_flyweight(key1.subject);
            ...
            }else if (value_str.empty() && !key_str.empty()){
                try{
...
//                return message;
            ...
            std::string message = "You have successfully reset database";
            return message;

        }else if (command_str.find("Restore database:") == 0) {
            try {
                //reset_database();
                parsing_in_file(key_str);
                std::string message = "You have successfully restore database";
                return message;
            } catch (const std::logic_error &er) {
                if (_logger != nullptr) _logger->log(er.what(), logger::severity::debug);
            }
        }else if (command_str.find("Saving state: ") == 0){
            if (_logger != nullptr) _logger->log("Saving state has started working", logger::severity::warning);
            try{
                saving_state(key_str);
                std::string message = "You have successfully saving state database";
                return message;
            }catch(const std::logic_error& er){
                throw er;
            }
            if (_logger != nullptr) _logger->log("Saving state has finished working", logger::severity::warning);
        }else{
