...
#ifndef SERVER_SPLAY_TREE_H
#define SERVER_SPLAY_TREE_H
...
//                    _tree->debug_print(right_subtree);
        ...
        void remove_after(const tkey & key, typename binary_search_tree<tkey, tvalue, tkey_comparer>::node** current_node,  std::stack<typename binary_search_tree<tkey, tvalue, tkey_comparer>::node*> *way_to_remove, logger* logger, size_t& side, size_t& dop) override{}
    };
public:
...
public:
    ...
    splay_tree<tkey, tvalue, tkey_comparer>& operator=( splay_tree<tkey, tvalue, tkey_comparer>&& other){
        if (*this != other){
            this->clear();
            this->_root = other._root();
            this->_logger = other._logger;
